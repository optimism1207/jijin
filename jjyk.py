...
def cost_hold_method(min, max, step, current_value, purchase_fee, total_now, share_hold_now): 
    for amount_add in range(min , max + 1 ,step): 
        cost_hold = (total_now + amount_add) / ( round(amount_add * (1 - purchase_fee)/ current_value,2) + share_hold_now)
        cost_hold = round(cost_hold, 4)
        rate_of_hold = ((amount_add * (1 - purchase_fee) + share_hold_now * current_value) / (amount_add + total_now) - 1) * 100 
        if rate_of_hold < 0:
            rate_of_return = math.fabs(rate_of_hold / 100 / (1 + rate_of_hold / 100) * 100)
        else:
            rate_of_return = 0
        #print(amount_add,cost_hold,rate_of_hold,rate_of_return)
